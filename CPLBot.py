...
class CPLBot(nextcord.Client):
    def __init__(self):
        super().__init__(allowed_mentions=nextcord.AllowedMentions(
            everyone=False), intents=nextcord.Intents.all())
    ...
    async def process_command(self, message):
        ...
        try:
            ...
            if cmd_specific_channels is not None and message.channel.id not in cmd_specific_channels and message.guild == CPL_GUILD_ID:
                raise Forbidden("This command can only be used in following channels : " +
                                ' '.join(f"<#{i}>" for i in cmd_specific_channels))
