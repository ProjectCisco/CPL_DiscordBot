...
class RankedModule(abcModule):
    ...
    async def update_player_rank(self, match : Match):
        for table in ['stats2', 'current_season']:
            old_ranks = self.rankPreviewer.get_current_rank_for_players(match, table)
            new_ranks = self.rankPreviewer.calc_new_ranks(match, old_ranks, table)
    ...
    async def cmd_current_season_stats(self, *args, channel, member, guild, **_):
        if args:
            target_name = ' '.join(args)
            target = get_member_by_name(target_name, guild or self.client)
            if not target:
...
class RankPreviewer:
    ...
    def get_current_rank_for_players(self, report : Report, table = 'stats2') -> List[Rating]:
        return [self.database.get_playerstats_by_id(report.gametype.value, pl.id, table).get_rating() for pl in
                report.players]
